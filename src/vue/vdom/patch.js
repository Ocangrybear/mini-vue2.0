...
export function patch(oldVnode, vnode){
  ...
  if(isRealElement) {
    ...
    // 不符合上面两种， 代表标签不一致 并且不是文本节点
    // 为了节点复用 所以直接把旧的虚拟dom对应的真实dom赋值给新的虚拟dom的el属性
    const el = (vnode.el = oldVnode.le)
...
function updateChildren(parent, oldCh, newCh) {
  ...
  let oldEndIndex = oldCh.length - 1 // 老儿子的结束下表
  let oldEndVnode = oldCh[oldEndIndex] // 老儿子的结束节点
  
  let newStartIndex = 0  // 新儿子的起始下标
  let newStartVnode = newCh[0] // 新儿子的第一个节点
  let newEndIndex = newCh.length - 1 // 新儿子的结束下表
  let newEndVnode = newCh[newEndIndex] // 新儿子的结束节点
